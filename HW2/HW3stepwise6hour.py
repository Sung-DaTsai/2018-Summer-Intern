...
for i in range(12):
    ...
    for j in range(466):
        ...
        train_y.append(data[place][480*i+j+14])
...
for i in range(len(test_x)):
    ans[i].append(test_y[i][place])
    ans[i].append((ans[i][1] - test_y[i][place])**2)
    ans[i].append((ans[i][1] - test_y[i][place])/test_y[i][place])
    total_loss += ans[i][3]
...
filename = "predict_6hour_Cost.csv"
text = open(filename, "w+")
s = csv.writer(text,delimiter=',',lineterminator='\n')
